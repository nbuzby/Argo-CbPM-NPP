...
% It is called by findmld.m.
%
%
% Uses Holte and Talley (2009) https://doi.org/10.1175/2009JTECHO543.1
%
% Updated 329 July 2021 - changed calc of sum to nansum to avoid output
% being NaN - Jacki Long (MBARI)
...
starti = min(find((pres-10).^2==min((pres-10).^2)));  
    %.^ element wise power raising)
pres = pres(starti:m);
sal = sal(starti:m);
temp = temp(starti:m);

%reset index values (1 is now the 10 dbar reference, and m is new length of
%profile data)
starti = 1;
m = length(sal); 
...
pdenseg = [pden(starti)-pden(mldepthdens(mldindex)-1) pden(starti) - ...
    pden(mldepthdens(mldindex))];
...
errort = errort/nansum(errort); 
errors = errors/nansum(errors);
errord = errord/nansum(errord);
...
% Algorithm mlds
mixedtp(mldindex) = mixedt(mldindex); %From temp alg
mixedsp(mldindex) = mixeds(mldindex); %From salinity alg
mixeddp(mldindex) = mixedd(mldindex); %From density alg

% Theshold method mlds
mldepthdensp(mldindex) = mldepthdens(mldindex); %From density threshold
mldepthptmpp(mldindex) = mldepthptmp(mldindex); %From temp threshold

% Gradient method mlds
gtmldp(mldindex) = pres(gtmld(mldindex)); %From temp gradient
gdmldp(mldindex) = pres(gdmld(mldindex)); %From density gradient
